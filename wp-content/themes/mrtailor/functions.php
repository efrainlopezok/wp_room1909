...
add_action('wp_head','mrtailor_catalogMode');
function mrtailor_catalogMode() {
	global $mr_tailor_theme_options;

	$mr_tailor_theme_options['catalog_mode'] = isset($mr_tailor_theme_options['catalog_mode']) ? $mr_tailor_theme_options['catalog_mode'] : 0;
?>
    <script type="text/javascript">
        var mrtailor_catalogMode = '<?php echo $mr_tailor_theme_options['catalog_mode']; ?>';
    </script>
<?php } ?>
<?php
...
/******************************************************************************/
/******************* Hide Empty Cart Page Title *******************************/
/******************************************************************************/

add_filter('woocommerce_cart_is_empty', 'getbowtied_hide_empty_cart_title');
function getbowtied_hide_empty_cart_title() {
...
if ( ! function_exists( 'getbowtied_output_related' ) ) {
	function getbowtied_output_related() {
	    ...
	    $mr_tailor_theme_options['related_products_per_view'] = isset($mr_tailor_theme_options['related_products_per_view']) ? $mr_tailor_theme_options['related_products_per_view'] : 4;
		
		echo '<div class="row">';
			echo '<div class="large-12 large-centered columns">';
		    $atts = array(
		    	'posts_per_page' => '12',
				'columns'		 => $mr_tailor_theme_options['related_products_per_view'],
				'orderby'        => 'rand',
				'order' 		 => 'asc'
			);
	    ...
	    echo '<script type="text/javascript">
	        var mrtailor_relatedProducts = ' . $mr_tailor_theme_options['related_products_per_view'] .';
	    </script>';
	}
}
